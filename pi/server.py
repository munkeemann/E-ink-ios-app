...
# sleeve_id -> {"active_idx": int, "cell": str}
...
# Firmware/app convention for active_idx in /zone_update + /zones (SAM1-68 adds CMD=5)
ZONE_INDEX_TO_CELL: dict[int, str] = {4: "LIB", 3: "HND", 2: "BTFLD", 1: "GRV", 0: "EXL", 5: "CMD"}
...
def zone_update():
    """Sleeve reports its current zone (SAM1-71). Body: {sleeve_id, active_idx, cell}."""
    data = request.get_json(force=True, silent=True) or {}
    sleeve_id = str(data.get("sleeve_id", "")).strip()
    active_idx = data.get("active_idx")
    cell = data.get("cell")

    if not sleeve_id or active_idx is None or cell is None:
        return jsonify(error="sleeve_id, active_idx, and cell required"), 400

    with zone_lock:
        zone_states[sleeve_id] = {"active_idx": int(active_idx), "cell": str(cell)}

    logging.info(f"Sleeve '{sleeve_id}' zone → active_idx={active_idx} cell={cell}")
    return jsonify(ok=True, sleeve_id=sleeve_id, active_idx=int(active_idx), cell=str(cell)), 200


@app.get("/zones")
def list_zones():
    """Returns last-known zone per registered sleeve. Defaults to LIB if unrecorded."""
    with registry_lock:
        registered = list(registry.keys())
    with zone_lock:
        snap = {
            sid: zone_states.get(sid, {"active_idx": 4, "cell": "LIB"})
...
def set_zone():
    ...
    try:
        ...
        resp.raise_for_status()
        zone_index = int(zone)
        cell = ZONE_INDEX_TO_CELL.get(zone_index, "EXL")
        with zone_lock:
            zone_states[sleeve_id] = {"active_idx": zone_index, "cell": cell}
        logging.info(f"Set sleeve '{sleeve_id}' zone to active_idx={zone_index} cell={cell}")
        return jsonify(ok=True), 200
    except requests.exceptions.RequestException as e:
        return jsonify(error=str(e)), 502
